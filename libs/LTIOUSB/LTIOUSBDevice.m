...
void _LTUSBInterfaceReadWriteAsyncCallback(void *refcon, IOReturn result, void *arg0)
{
    ...
    UInt32 size = (UInt32)(UInt64)arg0;
...
+(NSString *)deviceIdentifier:(io_service_t)device
{
    ...
    CFNumberRef productID = (CFNumberRef)CFDictionaryGetValue(dict, CFSTR(kUSBProductID) );
    CFNumberRef vendorID = (CFNumberRef)CFDictionaryGetValue(dict, CFSTR(kUSBVendorID) );
    CFStringRef serialNum = (CFStringRef)CFDictionaryGetValue(dict, CFSTR(kUSBSerialNumberString));
...
#warning @TODO: get pipe max size from endpoint descriptor
    ...
    UInt8* buf = (UInt8*)malloc(sizeof(UInt8)*pipeMaxSize);
...
-(BOOL)readFromPipeAsync:(UInt8)pipe callback:(LTIOUSBDeviceReadCallback)callback maxPacketSize:(UInt32)maxSize noDataTimeout:(UInt32)dto completionTimeout:(UInt32)cto
{
    ...
    UInt8* buffer = (UInt8*)malloc(sizeof(UInt8)*maxSize);
